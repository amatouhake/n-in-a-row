...
class Config:
    ...
    signs += "".join(set((ascii_uppercase + digits +
                          punctuation).strip(signs)))
...
class Game(ttk.Frame):
    def __init__(self, *args: tuple[Any], **kargs: dict[str, Any]) -> None:
        ...
        self.spaces = np.array(sorted(list(self.space_frame.generate_spaces(
            Config.shape, (), self.judge)), key=lambda x: x.coordinates)).reshape(Config.shape)

    def start_main(self, event: tk.Event = None) -> None:
        self.destroy()
        Main(self.master).pack()

    def judge(self, event: tk.Event) -> None:
        space = event.widget
        sign = Config.signs[self.player - 1]
        space.player = self.player

        label = ttk.Label(space, anchor="center",
                          background="light gray", text=sign)
        label.place(width=Config.space_size - Config.border_size * 2,
                    height=Config.space_size - Config.border_size * 2)
        space.label = label
